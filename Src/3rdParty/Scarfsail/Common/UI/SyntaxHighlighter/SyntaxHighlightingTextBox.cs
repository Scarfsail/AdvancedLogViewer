...
namespace Sarfsail.Common.UI.SyntaxHighlighter
{
    ...
    public class SyntaxHighlightingTextBox : System.Windows.Forms.RichTextBox
    {
        ...
        private bool mEnableHighlihting = true;
        private bool mEnableAutoCompleteForm = true;
        ...
        {
            get { return mEnableHighlihting; }
            set
            {
                mEnableHighlihting = value;
                if (value) Highlight();
                else UnHighlight();
            }
        }
        ...
        public int AddHighlightDescriptor(DescriptorRecognition dr, string token, DescriptorType descriptorType, Color color, Font font, bool useForAutoComplete)
        {
            return this.HighlightDescriptors.Add(new HighlightDescriptor(token, color, font, descriptorType, dr, useForAutoComplete));
        }

        /// <summary>
        /// Removes a highlight descriptor by its Index.        
        /// If the index doesn't exist, out of range exception is thrown
        /// </summary>
        /// <param name="index">ID of the descriptor to remove</param>
        public void RemoveHighlightDescriptiorById(int index)
        {
            this.HighlightDescriptors.RemoveAt(index);
        }
